...
# Must match your fork (raw script URL for CustomScript); validate-artifacts rejects mate-academy URLs.
$installScriptUrl = "https://raw.githubusercontent.com/NazarKulyk6/azure_task_12_deploy_app_with_vm_extention/main/install-app.sh"
...
New-AzVM -ResourceGroupName $resourceGroupName -Name $vmName -Location $location -Image $vmImage -Size $vmSize `
    ...
    -SshKeyName $sshKeyName -PublicIpAddressName $publicIpAddressName -Credential $cred

Write-Host "Installing todo app via CustomScript extension (URL from your fork) ..."
$extSettings = @{
    fileUris        = @($installScriptUrl)
    commandToExecute = "bash install-app.sh"
}
Set-AzVMExtension -ResourceGroupName $resourceGroupName -VMName $vmName -Location $location `
    -Name "InstallTodoApp" -Publisher "Microsoft.Azure.Extensions" -ExtensionType "CustomScript" `
    -TypeHandlerVersion "2.1" -Settings $extSettings

$pipInfo = Get-AzPublicIpAddress -ResourceGroupName $resourceGroupName -Name $publicIpAddressName
Write-Host "Done. Open: http://$($pipInfo.DnsSettings.Fqdn):8080/api/"
